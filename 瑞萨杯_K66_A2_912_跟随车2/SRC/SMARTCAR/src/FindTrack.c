...
static void SearchCenterBlackline(void)
{
    ...
    SetInitVal();
    //前几行，采用从中间往两边扫描的方法
    for(i=StartRow; i>=StartRow-5; i--) //首先找前行，全行扫描
    {
    ...
    for(i=StartRow-6; i>=25; i--)//查找剩余行
    {
        ...
        else if(LeftEdge[i+1]<effective_left && RightEdge[i+1]>effective_right)  //上一行没找到边界
        {
