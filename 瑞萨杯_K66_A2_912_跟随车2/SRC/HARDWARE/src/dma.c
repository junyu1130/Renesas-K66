 ...
 * @date       		2017-09-19
 ...
 添加不需要触发源的dma以及源地址为内存的dma - - 高源辰 2018.2.27
 ********************************************************************************************************************/
...
//                  DMA_EN(DMA_CH0);                				//需要使能 DMA 后才能传输数据
//-------------------------------------------------------------------------------------------------------------------
void dma_portx2buff_init_new(DMA_CHn CHn, void *SADDR, void *DADDR, DMA_BYTEn byten, uint32 count, uint32 cfg)
{
    ...
    DMAMUX0->CHCFG[CHn] = (0
                            | DMAMUX_CHCFG_ENBL_MASK                        // Enable routing of DMA request 
                            //| DMAMUX_CHCFG_TRIG_MASK                      // Trigger Mode: Periodic   PIT周期触发传输模式   通道1对应PIT1，必须使能PIT1，且配置相应的PIT定时触发 
                            | DMAMUX_CHCFG_SOURCE( DMA_Always_EN3)  // 通道触发传输源:    高源辰修改
                          );
...
//-------------------------------------------------------------------------------------------------------------------
//  @brief      DMA初始化，由IO口请求传输输入端口的数据到内存  源地址是变量
//  @param      DMA_CHn         通道号（DMA_CH0 ~ DMA_CH15）
//  @param      SADDR           源地址( (void * )&X_IN_DATA
//  @param      DADDR           目的地址
//  @param      PTxn            触发端口
...
//                  DMA_EN(DMA_CH0);                				//需要使能 DMA 后才能传输数据
//-------------------------------------------------------------------------------------------------------------------
void dma_portx2buff_init_new2(DMA_CHn CHn, void *SADDR, void *DADDR, PTX_n ptxn, DMA_BYTEn byten, uint32 count, uint32 cfg)
{
...
//    uint8 ptx0 = ((((uint32)SADDR) & 0x1C0) >> 6 ) << 5;
    ...
    DMA0->TCD[CHn].DOFF = 0;                                
    //DMA0->TCD[CHn].DOFF = BYTEs;                            // 每次传输后，目的地址加 BYTEs
		DMA0->TCD[CHn].DOFF = 0x00u;                            // 每次传输后，目的地址加 BYTEs
    DMA0->TCD[CHn].ATTR = 0;
    DMA0->TCD[CHn].ATTR = (0
                         | DMA_ATTR_SMOD(0x0)               // 源地址模数禁止  Source address modulo feature is disabled
                         | DMA_ATTR_SSIZE(byten)            // 源数据位宽 ：DMA_BYTEn  。    SSIZE = 0 -> 8-bit ，SSIZE = 1 -> 16-bit ，SSIZE = 2 -> 32-bit ，SSIZE = 4 -> 16-byte
