...
uint8_t NRF2401Channel = 10;

uint8_t NRF2401TXBuffer[TX_PLOAD_WIDTH]; //无线发送数组
uint8_t NRF2401RXBuffer[RX_PLOAD_WIDTH]; //无线接收数组
uint8_t WirelessReceiveStorage[32];
...
//	GPIO_Init(&GPIO_InitStruct1);
	gpio_init(NRF2401_CE_PORT,GPO,0);
...
//	GPIO_Init(&GPIO_InitStruct1);
	//gpio_init(NRF2401_IRQ_PORT,GPI,1);//下降沿触发中断
	disable_irq(PORTC_IRQn);                             		
  port_init(NRF2401_IRQ_PORT, ALT1 | IRQ_FALLING | PULLDOWN);        		
...
//	GPIO_Init(&GPIO_InitStruct1);
	gpio_init(NRF2401_SCK_PORT,GPO,0);
...
//	GPIO_Init(&GPIO_InitStruct1);
	gpio_init(NRF2401_SDI_PORT,GPI,1);
...
//	GPIO_Init(&GPIO_InitStruct1);
	gpio_init(NRF2401_SDO_PORT,GPO,0);
...
//	GPIO_Init(&GPIO_InitStruct1);
	gpio_init(NRF2401_CSN_PORT,GPO,0);
...
{
	NRF2401_CE=0;	  
	NVIC_EnableIRQ(PORTC_IRQn);//开启无线PTC中断 
	...
	NRF2401_ReadWriteReg(WRITE_REG+RF_CH,NRF2401Channel);//设置RF通信频率		  
...
{				
	NRF2401_CE=0;	    
	NVIC_DisableIRQ(PORTC_IRQn);//关闭无线PTC中断 
	...
	NRF2401_ReadWriteReg(WRITE_REG+EN_AA,0x01);     //使能通道0的自动应答    
	NRF2401_ReadWriteReg(WRITE_REG+EN_RXADDR,0x01); //使能通道0的接收地址  
	NRF2401_ReadWriteReg(WRITE_REG+SETUP_RETR,0x3f);//设置自动重发间隔时间:1000us + 86us;最大自动重发次数:15次
	NRF2401_ReadWriteReg(WRITE_REG+RF_CH,NRF2401Channel);//设置RF通道
	NRF2401_ReadWriteReg(WRITE_REG+RF_SETUP,0x0f);  //设置TX发射参数,0db增益,2Mbps,低噪声增益开启   
...
//2401双工发送
uint8_t NRF2401_SendData_Duplex(uint8_t *txbuf)
{
	uint8_t tmp = 0;
	//先将2401初始化为发送模式
	NRF2401_SetTXMode();
	//发送
	tmp = NRF2401_SendData(txbuf);
	//最后再发送完成之后将2401初始化为接受模式
	NRF2401_SetRXMode();
	return tmp;
}
