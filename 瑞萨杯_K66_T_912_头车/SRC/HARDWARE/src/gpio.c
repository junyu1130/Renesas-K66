 ...
 * @date       		2017-09-19
 ...
 更新说明：
 更新时间			修改人				修改说明
 2018.2.27		高源辰				增加gpio口输入模式时的配置	dat 0-浮空 1-下拉 2-上拉
 ********************************************************************************************************************/
...
//  @return     void
//  @since      v1.0
//  Sample usage:               gpio_init(A5,GPO,0);   //初始化A5为gpio模式，设置为输出模式，浮空输入
//-------------------------------------------------------------------------------------------------------------------
void gpio_init(PTX_n ptx_n, GPIO_MOD ddr, uint8 dat)
{
    ...
    if(ddr)
    {
		...
		else//输入模式配置 ——高源辰，2018.2.27
		{
			if(dat == 0)//浮空输入
			{
				PORTX[ptx]->PCR[ptn] &=~PORT_PCR_PE_MASK; 	//上下拉电阻DISABLE
			}
			else if(dat == 1)//下拉
			{
				PORTX[ptx]->PCR[ptn] |= PORT_PCR_PE_MASK; 	//上下拉电阻使能
				PORTX[ptx]->PCR[ptn] &= ~PORT_PCR_PS_MASK;
			}
			else if(dat == 2)//上拉
			{
				PORTX[ptx]->PCR[ptn] |= PORT_PCR_PE_MASK; 	//上下拉电阻使能
