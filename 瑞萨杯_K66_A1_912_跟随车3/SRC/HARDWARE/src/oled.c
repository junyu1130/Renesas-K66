...
//    OLED_Pin_Init(OLED_PORT, OLED_DC);     //GPIO_Init(OLED_PORT, OLED_DC, OUT, 1);     //配置OLED端口引脚为GPIO输出
	...
	//重写gpio初始化
	OLED_Pin_Init(OLED_SCL);     //GPIO_Init(OLED_PORT, OLED_SCL, OUT, 1);
	OLED_Pin_Init(OLED_SDA);     //GPIO_Init(OLED_PORT, OLED_SDA, OUT, 1);
	OLED_Pin_Init(OLED_RST);     //GPIO_Init(OLED_PORT, OLED_RST, OUT, 1);
	OLED_Pin_Init(OLED_DC);     //GPIO_Init(OLED_PORT, OLED_DC, OUT, 1);     //配置OLED端口引脚为GPIO输出
...
/*void OLED_Write_Picture(uint8_t*pic)
{
	uint16_t i,j;
	OLED_Set_XY(0,0);
	for(j=0;j<=8;j++)
...
{
		uint16_t i,j;
		uint16_t line=wordSize/8;
		chn=chn+(num-1)*wordSize*line;
		for(j=0;j<line;j++)
		{
			OLED_Set_XY(x * 8, y+j);
