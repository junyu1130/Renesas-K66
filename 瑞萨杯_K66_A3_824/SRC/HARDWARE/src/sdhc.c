...
//  @note       内部使用，用户无需调用
//-------------------------------------------------------------------------------------------------------------------
SDHCRES sdhc_sendcommand(ESDHC_COMMAND_STRUCT_PTR command)
{
    uint32 xfertyp;
    uint32 blkattr;
...
//  @note       
//-------------------------------------------------------------------------------------------------------------------
SDHCSTATUS sdhc_initcard(void)//不使用
{
...
//  @note       
//-------------------------------------------------------------------------------------------------------------------
SDHCSTATUS sdhc_initcard_25(void)//25000000
{
    uint32 param, c_size, c_size_mult, read_bl_len, time_out = 0;
    ESDHC_COMMAND_STRUCT command;
    SDHCSTATUS result;
...
SDHCSTATUS sdhc_initcard_40(void)//40000000
{
    uint32 param, c_size, c_size_mult, read_bl_len, time_out = 0;
    ESDHC_COMMAND_STRUCT command;
    SDHCSTATUS result;
    
		//OLED_P6x8Str(0,0,(uint8_t*)"SD INIT in");
		
		//while(1);//debug
	
    //分配SD卡信息结构体的数据空间并初始化
    //sdcard_ptr = (SDCARD_STRUCT_PTR)malloc(sizeof(SDCARD_STRUCT));
	//OLED_Print_Num(0,2,sizeof(SDCARD_STRUCT));	
	//sdcard_ptr = (SDCARD_STRUCT_PTR)malloc(24);
	//while(1);//debug
	sdcard_ptr = &sdcard_temp;
    sdcard_ptr->CARD = ESDHC_CARD_NONE;
    sdcard_ptr->TIMEOUT = 0;
    sdcard_ptr->NUM_BLOCKS = 0;
    sdcard_ptr->ADDRESS = 0;
    ...
    if(time_out >= 1000)
		{
			//OLED_P6x8Str(0,0,(uint8_t*)"SDHCRES_NOTRDY");
			return SDHCRES_NOTRDY;
		}
