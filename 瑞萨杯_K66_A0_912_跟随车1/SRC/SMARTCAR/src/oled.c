    ...
    for(i=0; i<temp; i++)
    {
        ...
        for(j=0; j<width; j++)
        {
            ...
            if( i<(temp-1) || !temp1 || temp1>=2)
                dat |= (*(p+i*8*width+j+width*1) > value? 1: 0)<<1;
            if( i<(temp-1) || !temp1 || temp1>=3)
                dat |= (*(p+i*8*width+j+width*2) > value? 1: 0)<<2;
            if( i<(temp-1) || !temp1 || temp1>=4)
                dat |= (*(p+i*8*width+j+width*3) > value? 1: 0)<<3;
            if( i<(temp-1) || !temp1 || temp1>=5)
                dat |= (*(p+i*8*width+j+width*4) > value? 1: 0)<<4;
            if( i<(temp-1) || !temp1 || temp1>=6)
                dat |= (*(p+i*8*width+j+width*5) > value? 1: 0)<<5;
            if( i<(temp-1) || !temp1 || temp1>=7)
                dat |= (*(p+i*8*width+j+width*6) > value? 1: 0)<<6;
            if( i<(temp-1) || !temp1 || temp1>=8)
                dat |= (*(p+i*8*width+j+width*7) > value? 1: 0)<<7;
            OLED_WrDat(dat);
        }
    }
}
...
void dis_mybmp(uint16_t high, uint16_t width, uint8_t *p)
{
    ...
    for(i=0; i<temp; i++)
    {
        ...
        for(j=0; j<width; j++)
        {
            dat = 0;
            if( i<(temp-1) || !temp1 || temp1>=1)
                dat |= (*(p+i*8*width+j+width*0))<<0;
            if( i<(temp-1) || !temp1 || temp1>=2)
                dat |= (*(p+i*8*width+j+width*1))<<1;
            if( i<(temp-1) || !temp1 || temp1>=3)
                dat |= (*(p+i*8*width+j+width*2))<<2;
            if( i<(temp-1) || !temp1 || temp1>=4)
                dat |= (*(p+i*8*width+j+width*3))<<3;
            if( i<(temp-1) || !temp1 || temp1>=5)
                dat |= (*(p+i*8*width+j+width*4))<<4;
            if( i<(temp-1) || !temp1 || temp1>=6)
                dat |= (*(p+i*8*width+j+width*5))<<5;
            if( i<(temp-1) || !temp1 || temp1>=7)
                dat |= (*(p+i*8*width+j+width*6))<<6;
            if( i<(temp-1) || !temp1 || temp1>=8)
                dat |= (*(p+i*8*width+j+width*7))<<7;
            OLED_WrDat(dat);
        }
    }
}
